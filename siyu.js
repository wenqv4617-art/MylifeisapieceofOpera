...
db.version(1).stores({
  ...
  messages: "++id, chatId, role, text, avatar, name, createdAt, fav, [chatId+createdAt]",
  state: "key"
});
...
const messages = await db.messages
  ...
  .orderBy("createdAt")  // 使用 orderBy，让数据库排序
  .reverse()             // 反转得到最新的在前面
  .limit(50)            // 限制 50 条
  .toArray();            // 直接转数组，已按 createdAt 降序
  // ✅ 新代码（从旧到新追加，保持正确顺序）：
const box = main.querySelector("#siyuMessages");
const frag = document.createDocumentFragment();

// 因为 messages 已经是降序（最新在前），所以要倒序遍历
for (let i = messages.length - 1; i >= 0; i--) {
  frag.appendChild(messageNode(messages[i]));
}
...
requestAnimationFrame(() => {
  ...
  // 构造头像和气泡内容
  const avatarHtml = `<div class="siyu-bubble-avatar">${avatarHTML(m.avatar)}</div>`;
  ...
  // 根据角色决定顺序：char 头像在左，user 头像在右
  if (m.role === "user") {
    // user：bubble-wrap 在左，avatar 在右
    node.innerHTML = bubbleWrapHtml + avatarHtml;
  } else {
    // char：avatar 在左，bubble-wrap 在右
    node.innerHTML = avatarHtml + bubbleWrapHtml;
  }
