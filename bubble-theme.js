...
async function applyGlobalTheme(themeId) {
  ...
  /*
   * 这里不做作用域隔离。
   * 用户导入的 CSS 会作为全局样式直接注入 head，
   * 因此可以覆盖首页、聊天室、联系人、世界书、群聊、日记、论坛等所有页面。
   */
...
if (gtRow && gtRow.querySelector(".gt-load, .gt-apply, .gt-unload, .gt-rename, .gt-del")) {
  ...
  if (t.classList.contains          const theme = await window.DB.get(STORE_NAME, id);
    if (!theme) return;
